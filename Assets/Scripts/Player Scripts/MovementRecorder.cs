...
public class MovementRecorder : MonoBehaviour
{
    ...
    private Dictionary<GameObject, Stack<(Vector3, Quaternion)>> bodyPartTransformations; 
    public Dictionary<GameObject, Stack<(Vector3, Quaternion)>> BodyPartTransformations { get { return bodyPartTransformations; } }
...
/*         if(!playerCollision.IsPlayerDead){
    ...
    public void RewindPlayerLastTransformation(){
        foreach(GameObject bodyPart in playerBodyGameObjects){
            if(bodyPartTransformations[bodyPart].Count > 0){
                (Vector3, Quaternion) lastTransformation = bodyPartTransformations[bodyPart].Pop();
    ...
    public (Vector3, Quaternion) GetLastTransformationForBodyPart(GameObject bodyPart){
        return bodyPartTransformations[bodyPart].Pop();
    }

    private void InitializeDictionary(){
        bodyPartTransformations = new Dictionary<GameObject, Stack<(Vector3, Quaternion)>>();

        foreach(GameObject bodyPart in playerBodyGameObjects){
            Stack<(Vector3, Quaternion)> bodyPartTransformation = new();
            bodyPartTransformation.Push((bodyPart.transform.localPosition, bodyPart.transform.localRotation));
    ...
    private void AddBodyPartCurrentTransformation(){
        foreach(GameObject bodyPart in playerBodyGameObjects){
            bodyPartTransformations[bodyPart].Push((bodyPart.transform.localPosition, bodyPart.transform.localRotation));
        }
    }
}
